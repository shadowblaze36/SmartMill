...
def login_process():
                    ...
                    if name1=='':
                         ...
                         cursor.execute(sql)
                         if cursor.fetchall():
                                              os.system("sudo python /home/pi/Panel_molino2.py")
                         else:
...
image_logo=PhotoImage(file="usap_logo2.png")
button_logo=Button(v0,image=image_logo).place(x=100,y=100,width=200,height=70)
image_usap=PhotoImage(file="icon.png")
button=Button(v0,image=image_usap).place(x=10,y=10,width=60,height=65)
# --- Labels ---------
textfont=tkFont.Font(family="Times New Roman",size=18,weight="bold")
textfont2=tkFont.Font(family="Times New Roman",size=14)
label1=Label(v0,text="USAP SMART MILL",font=textfont).place(x=80,y=10)
...
textbox2=Entry(v0,textvariable=password,width=15,show="*").place(x=370,y=170)
#---- Buttons ---------
