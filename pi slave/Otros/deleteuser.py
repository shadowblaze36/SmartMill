...
v0=Tk()
v0.title("Deactivate users --- Security Door ---")
...
def save():
           ...
           retry1=retry.get()
           statusf=status.get()
           status1='I' # for to disactive or to eliminate our user.
           status.set(status1)
           #--- field's validation-----
           if password1!=retry1:
                         ...
                         tkMessageBox.showinfo("save",message="Empty job")
                             ...
                             tkMessageBox.showinfo("save",message="Empty journal")
                              ...
                              tkMessageBox.showinfo("save",message="already inactive user")
           else:                 
                ...
                label_save=Label(v0,text="Deactivated user",font=textfont).place(x=310,y=315)
                # ---- cleaning fields --------
                name.set(str(''))
                lastname.set(str(''))
                startdate.set(str(''))
...
label=Label(v0,text="Deactivate User",font=text_title).place(x=200,y=10)
...
textbox1=Entry(v0,textvariable=name,width=20,state='disabled').place(x=120,y=95)
textbox2=Entry(v0,textvariable=lastname,width=20,state='disabled').place(x=120,y=125)
textbox3=Entry(v0,textvariable=startdate,width=20,state='disabled').place(x=120,y=155)
textbox4=Entry(v0,textvariable=job,width=20,state='disabled').place(x=120,y=185)
textbox5=Entry(v0,textvariable=journal,width=20,state='disabled').place(x=120,y=215)
textbox6=Entry(v0,textvariable=password,width=20,state='disabled').place(x=120,y=245)
textbox7=Entry(v0,textvariable=retry,width=20,state='disabled').place(x=165,y=275)
textbox8=Entry(v0,textvariable=status,width=2,state='disabled').place(x=270,y=314)
...
save_image=PhotoImage(file="delete.gif")
